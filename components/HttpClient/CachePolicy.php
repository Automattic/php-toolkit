...
final class CachePolicy {
	...
	public static function response_is_cacheable( Response $r ) : bool {
		...
		if ( $r->status_code !== 200 && $r->status_code !== 206 ) return false;

		$d = self::directives( $r->get_header( 'cache-control' ) );
		if ( isset( $d['no-store'] ) ) return false;
	...
	public static function freshness_lifetime( CacheEntry $e ) : int {
		...
		if ( isset( $d['s-maxage'] ) ) return $d['s-maxage'];
		if ( isset( $d['max-age'] ) ) return $d['max-age'];
		if ( isset( $h['expires'] ) ) return max( 0, strtotime( $h['expires'] ) - $e->stored_at );
