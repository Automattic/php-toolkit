...
class BlockDiffMergeDriver {
    ...
    private $p;
    private $i;
    private $block_delimiter;
    private $diff;
    private $change;
    private $change_type;
    private $change_string;
    private $change_length;
    private $cursor_in_original_doc;
    public function diff_cleanup_block_boundaries4( $document_a, $diff ) {
        $annotated_diff = [];
        $cursor_in_original_doc = 0;
        $cursor_in_updated_doc = 0;
        foreach($diff as $change) {
        ...
        $this->p = new BlockMarkupProcessor($document_a);
        if(false === $this->next_block_delimiter()) {
            return $diff;
        }
        $delimiter_idx = 0;
        ...
        while(count($annotated_diff)) {
            ...
            // skip until the next relevant block delimiter.
            while(
                $this->block_delimiter &&
                $change['original_doc']['start'] >= $this->block_delimiter->start + $this->block_delimiter->length
            ) {
                $this->next_block_delimiter();
                ++$delimiter_idx;
            }

            // If no more block delimiters, just add the change
            if(!$this->block_delimiter) {
                $split_diff[] = [$change['type'], $change['string']];
                continue;
            }

            if($change['type'] === Diff::INSERT) {
                $accumulated_shift += mb_strlen($change['string']);
            }

            $intersects_block_delimiter = (
                // Ends before the block delimiter starts
                $change['original_doc']['start'] + $change['original_doc']['length'] < $this->block_delimiter->start ^
                // Starts after the block delimiter ends
                $this->block_delimiter->start + $this->block_delimiter->length > $change['original_doc']['start']
            );

            if($intersects_block_delimiter) {
                $block_delimiter_start_offset = $this->block_delimiter->start - $change['original_doc']['start'];
                $before_block_delimiter = mb_substr($change['string'], 0, $block_delimiter_start_offset);

                $block_delimeter_last_char_offset = $this->block_delimiter->start + $this->block_delimiter->length - $change['original_doc']['start'];
                $inside_block_delimiter = mb_substr($change['string'], $block_delimiter_start_offset, $block_delimeter_last_char_offset - $block_delimiter_start_offset);

                $after_block_delimiter = mb_substr($change['string'], mb_strlen($before_block_delimiter) + mb_strlen($inside_block_delimiter));
                print_r([
                    'change start' => $change['original_doc']['start'],
                    'block delimiter start' => $this->block_delimiter->start,
                    'block delimiter end' => $this->block_delimiter->start + $this->block_delimiter->length,
                    'before' => $before_block_delimiter,
                    'inside' => $inside_block_delimiter,
                    'after' => $after_block_delimiter,
                ]);
                ...
                // Store the part before the block delimiter
                if(mb_strlen($before_block_delimiter)) {
                    $split_diff[] = [$change['type'], $before_block_delimiter];
                }
                if(mb_strlen($inside_block_delimiter)) {
                    $split_diff[] = [$change['type'], $inside_block_delimiter, $delimiter_idx];
                }
                ...
                if(mb_strlen($after_block_delimiter) > 0) {
                    array_unshift($annotated_diff, [
                        ...
                        'updated_doc' => [
                            'start' => $change['updated_doc']['start'] + (
                                $change['updated_doc']['length'] > 0 ? mb_strlen($before_block_delimiter) : 0
                            ),
                            'length' => $change['updated_doc']['length'] > 0 ? mb_strlen($after_block_delimiter) : 0,
                        ],
                    ]);
        ...
        // Merge entries related to the same block delimiter
        $reduced_split_diff = [];
        foreach ($split_diff as $entry) {
            if (!empty($reduced_split_diff) && isset($entry[2]) && isset(end($reduced_split_diff)[2]) && $entry[2] === end($reduced_split_diff)[2]) {
                $last_entry = array_pop($reduced_split_diff);
                $last_entry[1] .= $entry[1];
                $reduced_split_diff[] = $last_entry;
            } else {
                $reduced_split_diff[] = $entry;
            }
        }
    ...
    public function diff_cleanup_block_boundaries3( $document_a, $diff ) {
        $this->p = new BlockMarkupProcessor($document_a);
        $this->diff = $diff;
        $this->i = -1;
        ...
        $this->next_diff_change();

        // Split the diffs on the block delimiter boundary
        $block_delimiters = [];
        $split_diff = [];
        while(true) {
            if(null === $this->change) {
            ...
            while(true) {
                ...
                if($this->cursor_in_original_doc >= $this->block_delimiter->start + $this->block_delimiter->length) {
                    $this->next_block_delimiter();
                    continue;
                }
                break;
            ...
            // If no more block delimiters, just add the change
            if(false === $this->block_delimiter) {
                $split_diff[] = $this->change;
                $this->next_diff_change();
                continue;
            }

            // The diff is entirely before the block delimiter
            if($this->cursor_in_original_doc + $this->change_length < $this->block_delimiter->start) {
                $split_diff[] = $this->change;
                $this->next_diff_change();
                continue;
            }

            // If the block delimiter is entirely contained within the diff,
            // we can just add the diff to the updated diff.
            // if($this->cursor_in_original_doc < $this->block_delimiter->start && $this->cursor_in_original_doc + $this->change_length > $this->block_delimiter->start + $this->block_delimiter->length) {
            //     $split_diff[] = $this->change;
            //     continue;
            // }
            // Otherwise, the diff overlaps with the block delimiter.
            // Let's split it into:
            // 1. Before block delimiter – whatever the diff was
            // 2. Delete the block delimiter and insert the updated one
            // 3. After block delimiter – whatever the diff was

            // If the diff crosses the start index of the block delimiter,
            // preserve the part before the block delimiter.
            $block_delimiter_start = count($split_diff);
            if($this->cursor_in_original_doc < $this->block_delimiter->start) {
                $split_diff[] = [
                    $this->change_type,
                    mb_substr($this->change_string, 0, $this->block_delimiter->start - $this->cursor_in_original_doc),
                ];
                ++$block_delimiter_start;
                $split_diff[] = [
                    $this->change_type,
                    mb_substr($this->change_string, $this->block_delimiter->start - $this->cursor_in_original_doc),
                ];
                if(false === $this->next_diff_change()) {
                    break;
                }
            }

            while(true) {
                // Skip all the changes contained within the block delimiter.
                if(
                    $this->cursor_in_original_doc >= $this->block_delimiter->start &&
                    $this->cursor_in_original_doc + $this->change_length <= $this->block_delimiter->start + $this->block_delimiter->length
                ) {
                    $split_diff[] = $this->change;
                    if(false === $this->next_diff_change()) {
                        break 2;
            ...
            $block_delimiter_end = count($split_diff) - 1;

            // If the last change goes past the block delimiter,
            // accumulate the part before the block delimiter.
            if(
                $this->cursor_in_original_doc < $this->block_delimiter->start + $this->block_delimiter->length &&
                $this->cursor_in_original_doc + $this->change_length > $this->block_delimiter->start + $this->block_delimiter->length
            ) {
                $last_change = [
                    $this->change_type,
                    mb_substr($this->change_string, 0, $this->block_delimiter->start + $this->block_delimiter->length - $this->cursor_in_original_doc),
                ];
                $split_diff[] = $last_change;
                ++$block_delimiter_end;

                // v this may contain another block delimiter, we need to reprocess
                $rest_change = [
                    $this->change_type,
                    mb_substr($this->change_string, $this->block_delimiter->start + $this->block_delimiter->length - $this->cursor_in_original_doc)
                ];
                if(mb_strlen($rest_change[1]) > 0) {
                    array_splice($this->diff, $this->i + 1, 0, [$rest_change]);
                }

                $this->change = $last_change;
                $this->change_type = $last_change[0];
                $this->change_string = $last_change[1];
                $this->change_length = mb_strlen($this->change_string);
                if(false === $this->next_diff_change()) {
                    break;
                }
            }

            $block_delimiters[$block_delimiter_start] = [
                'doc_start' => $this->block_delimiter->start,
                'doc_length' => $this->block_delimiter->length,
                'first_change_idx' => $block_delimiter_start,
                'last_change_idx' => $block_delimiter_end,
            ];
        }

        if(!count($block_delimiters)) {
            return $split_diff;
        }

        $updated_diff = [];
        for($i = 0; $i < count($split_diff); $i++) {
            if(!isset($block_delimiters[$i])) {
                $updated_diff[] = $split_diff[$i];
                continue;
            }
            
            $block_delimiter = $block_delimiters[$i];

            $old_block_delimiter = substr($document_a, $block_delimiter['doc_start'], $block_delimiter['doc_length']);
            $new_block_delimiter = '';
            for($i = $block_delimiter['first_change_idx']; $i <= $block_delimiter['last_change_idx']; $i++) {
                switch($split_diff[$i][0]) {
                    case Diff::EQUAL:
                    case Diff::INSERT:
                        $new_block_delimiter .= $split_diff[$i][1];
                        break;
                }
            }
            $updated_diff[] = [
            ...
            $i = $block_delimiter['last_change_idx'];
        }
        return $updated_diff;
    }

    public function diff_cleanup_block_boundaries2( $document_a, $document_b, $diff ) {
        $this->p = new BlockMarkupProcessor($document_a);
        $this->diff = $diff;
        $this->i = -1;
        $this->cursor_in_original_doc = 0;
        ...
        while($this->next_diff_change()) {
            ...
            while(true) {
                ...
                if(
                    ...
                    $new_block_delimiter[] = $this->change;
                    $this->next_diff_change();
                } else {
                    break;
                }
            ...
            // Append the block delimiter deletion and insertion
            $old_block_delimiter_string = substr($document_a, $this->block_delimiter->start, $this->block_delimiter->length);
            $new_block_delimiter_string = '';
            foreach($new_block_delimiter as $change) {
                switch($change[0]) {
                    case Diff::EQUAL:
                    case Diff::INSERT:
                        $new_block_delimiter_string .= $change[1];
                        break;
                }
            }
            $updated_diff[] = [
                Diff::DELETE,
                $old_block_delimiter_string,
            ];
            $updated_diff[] = [
                Diff::INSERT,
                $new_block_delimiter_string,
            ...
            // If the last change goes past the block delimiter,
            // preserve the part after the block delimiter.
            if(
                $this->cursor_in_original_doc < $this->block_delimiter->start + $this->block_delimiter->length &&
                $this->cursor_in_original_doc + $this->change_length > $this->block_delimiter->start + $this->block_delimiter->length
            ) {
                $this->change_length = $this->block_delimiter->start + $this->block_delimiter->length - $this->cursor_in_original_doc;
                $rest = mb_substr($this->change_string, $this->change_length);
                array_splice($this->change, $this->i, 0, [
                    $this->change_type,
                    $rest
                ]);
            }
    ...
    private function next_diff_change() {
        // Move the cursor_in_original_doc past the last change
        switch($this->change_type) {
            case Diff::EQUAL:
            case Diff::DELETE:
                $this->cursor_in_original_doc += $this->change_length;
                break;
        }
        $this->change = null;
        $this->change_type = null;
        $this->change_string = null;
        $this->change_length = 0;

        ++$this->i;
        if($this->i >= count($this->diff)) {
            return false;
        }
        $this->change = $this->diff[$this->i];
        $this->change_type = $this->change[0];
        $this->change_string = $this->change[1];
        $this->change_length = mb_strlen($this->change_string);
        return true;
    }
