...
class LocalFilesystemTest extends TestCase {
    ...
    protected function setUp(): void {
        $this->test_dir = sys_get_temp_dir() . '/wp-filesystem-test-' . uniqid();
        mkdir($this->test_dir);
    ...
    public function testLs() {
        ...
        file_put_contents($this->test_dir . '/test-file.txt', 'test');

        $contents = $this->fs->ls('/');
        sort($contents);

        $this->assertEquals(['test-dir', 'test-file.txt'], $contents);
    }
    ...
    public function testMkdir() {
        ...
        $this->assertTrue(is_dir($this->test_dir . '/new-dir'));
    ...
    public function testMkdirRecursive() {
        ...
        $this->assertTrue(is_dir($this->test_dir . '/parent/child/grandchild'));
    }

    public function testRmRemovesExistingFile() {
        file_put_contents($this->test_dir . '/test.txt', 'test');
        $this->fs->rm('/test.txt');
        $this->assertFalse(file_exists($this->test_dir . '/test.txt'));
    }
    ...
    public function testRmdirRecursive() {
        mkdir($this->test_dir . '/parent/child/grandchild', 0777, true);
        file_put_contents($this->test_dir . '/parent/test.txt', 'test');

        $this->fs->rmdir('/parent', ['recursive' => true]);
        $this->assertFalse(is_dir($this->test_dir . '/parent'));
    }
    ...
    public function testCopyDirectoryRecursively() {
        ...
        mkdir($this->test_dir . '/src/subdir', 0777, true);
        file_put_contents($this->test_dir . '/src/file1.txt', 'content1');
        file_put_contents($this->test_dir . '/src/subdir/file2.txt', 'content2');

        $this->fs->copy('/src', '/dest', ['recursive' => true]);

        // Verify directory structure was copied
        $this->assertTrue(is_dir($this->test_dir . '/dest'));
        $this->assertTrue(is_dir($this->test_dir . '/dest/subdir'));
        $this->assertTrue(is_file($this->test_dir . '/dest/file1.txt'));
        $this->assertTrue(is_file($this->test_dir . '/dest/subdir/file2.txt'));

        // Verify file contents
        $this->assertEquals('content1', file_get_contents($this->test_dir . '/dest/file1.txt'));
        $this->assertEquals('content2', file_get_contents($this->test_dir . '/dest/subdir/file2.txt'));
    }

    public function testCopyDirectoryFailsWhenSourceDoesNotExist() {
        $this->expectException(FilesystemException::class);
        $this->fs->copy('/nonexistent', '/dest', ['recursive' => true]);
    }
