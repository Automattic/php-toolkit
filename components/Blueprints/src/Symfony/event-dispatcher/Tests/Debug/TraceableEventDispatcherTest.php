...
class TraceableEventDispatcherTest extends TestCase {
	...
	public function testGetCalledListenersNested() {
		...
		$dispatcher->addListener(
			'foo',
			function ( Event $event, $eventName, $dispatcher ) use ( &$tdispatcher ) {
	...
	public function testLogger() {
		...
		$logger->expects( $this->at( 0 ) )->method( 'debug' )->with(
		...
		$logger->expects( $this->at( 1 ) )->method( 'debug' )->with(
			'Notified event "{event}" to listener "{listener}".',
			array(
				'event' => 'foo',
				'listener' => 'closure',
	...
	public function testLoggerWithStoppedEvent() {
		...
		$logger->expects( $this->at( 2 ) )->method( 'debug' )->with(
	...
	public function testDispatchReusedEventNested() {
		...
		$dispatcher->addListener(
			'foo',
			function ( Event $e ) use ( $dispatcher ) {
				$dispatcher->dispatch( 'bar', $e );
			}
		);
		$dispatcher->addListener(
			'bar',
			function ( Event $e ) use ( &$nestedCall ) {
				$nestedCall = true;
			}
		);
