...
class InstallPluginStep implements StepInterface {
     ...
     */
    public PluginErrorBehavior $onError;
...
class ExistingSiteResolver
{
    static public function resolve(Runtime $runtime, Tracker $targetResolutionStage)
    {
        ...
        try {
            $result = FilesystemHelpers::withTemporaryDirectory($targetFs, function($temp_dir) use ($targetFs, $runtime) {
        ...
        if (isset($blueprint['wordpressVersion'])) {
            ...
            $currentWordPressVersion = FilesystemHelpers::withTemporaryDirectory($targetFs, function($temp_dir) use ($targetFs, $runtime) {
        ...
        if ($requiredEngine === 'sqlite') {
            $sqliteActive = FilesystemHelpers::withTemporaryDirectory($targetFs, function($temp_dir) use ($targetFs, $runtime) {
            ...
            $usingMysql = FilesystemHelpers::withTemporaryDirectory($targetFs, function($temp_dir) use ($targetFs, $runtime) {
