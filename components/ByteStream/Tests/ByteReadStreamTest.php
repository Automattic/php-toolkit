...
class ByteReadStreamTest extends TestCase {
    ...
    public function byteReaderProvider() {
        ...
        return [
            'ResourceReader' => [FileReadStream::from_path( dirname(__FILE__) . '/fixtures/preface-to-pygmalion.txt'), strlen($text)],
            'StringReader' => [new MemoryPipe($text), strlen($text)],
            'InflateReader' => [new InflateReadStream(new MemoryPipe($compressedData)), strlen($text)],
            'DeflateReader' => [new DeflateReadStream(new MemoryPipe($text)), strlen($text)],
        ];
    }
